<?php
// Sidebar partial for Petrokimia admin
?>
<!-- Sidebar Collapse Styles -->
<style>
    #sidebar {
        transition: width 0.3s ease-in-out;
        position: relative;
        width: 80px;
    }

    #sidebar:hover {
        width: 256px;
    }

    .sidebar-label {
        display: none;
        white-space: nowrap;
    }

    #sidebar:hover .sidebar-label {
        display: inline;
    }

    .sidebar-header {
        display: flex;
        align-items: center;
        justify-content: center;
        padding: 0;
        height: 80px;
        transition: all 0.3s ease-in-out;
        gap: 12px;
    }

    #sidebar:hover .sidebar-header {
        justify-content: center;
        padding: 0 16px;
        height: 80px;
    }

    .nav-item {
        transition: all 0.3s ease-in-out;
        justify-content: center;
        padding: 16px 0 !important;
        margin-left: 0 !important;
        margin-right: 0 !important;
        gap: 0;
    }

    #sidebar:hover .nav-item {
        justify-content: flex-start;
        padding: 12px 16px !important;
        margin-left: 0 !important;
        margin-right: 0 !important;
        gap: 12px;
    }

    .nav-item-text {
        display: none;
    }

    #sidebar:hover .nav-item-text {
        display: inline;
    }

    .logout-text {
        display: none;
    }

    #sidebar:hover .logout-text {
        display: inline;
    }

    .nav-item svg {
        min-width: 24px;
        width: 24px;
        height: 24px;
        flex-shrink: 0;
        margin-right: 0;
    }

    #sidebar:hover .nav-item svg {
        margin-right: 0;
    }

    .sidebar-header svg {
        min-width: 32px;
        width: 32px;
        height: 32px;
        flex-shrink: 0;
    }

    .nav-item {
        white-space: nowrap;
    }

    #sidebar nav {
        scrollbar-width: none;
    }

    #sidebar nav::-webkit-scrollbar {
        display: none;
    }
</style>

<!-- Sidebar (Hidden on mobile, visible on desktop) -->
<aside id="sidebar" class="hidden md:flex flex-col bg-gray-900 text-white">
    <div class="sidebar-header flex items-center h-20 border-b border-gray-700">
        <i class="fa-solid fa-cloud-arrow-up text-2xl"></i>
        <span class="sidebar-label text-2xl font-semibold">Petrokimia</span>
    </div>
    <nav class="flex-1 overflow-y-auto px-4 py-6" tabindex="0">
        <?php
        if (function_exists('render_partial')) {
            render_partial('menu_petrokimia', ['mobile' => false]);
        } else {
            $mobile = false;
            include __DIR__ . '/menu_petrokimia.php';
        }
        ?>
    </nav>
    <div class="p-4 border-t border-gray-700">
        <?php if (is_superadmin()): ?>
            <a href="/dashboard/superadmin/dashboard.php" class="nav-item flex items-center px-4 py-3 text-gray-300 hover:bg-gray-700 hover:text-white rounded-lg">
                <i class="fa-solid fa-right-from-bracket text-lg"></i>
                <span class="logout-text">Dashboard Superadmin</span>
            </a>
        <?php endif; ?>
        <?php if (is_adminpk()): ?>
            <a href="../../logout.php" class="nav-item flex items-center px-4 py-3 text-gray-300 hover:bg-gray-700 hover:text-white rounded-lg">
                <i class="fa-solid fa-right-from-bracket text-lg"></i>
                <span class="logout-text">Keluar</span>
            </a>
        <?php endif; ?>
    </div>
</aside>

<!-- Sidebar Mobile (Initially hidden) -->
<div id="mobile-menu" class="fixed inset-0 z-30 flex md:hidden transform -translate-x-full transition-transform duration-300 ease-in-out opacity-0" role="dialog" aria-modal="true" aria-hidden="true">
    <div class="relative w-64 max-w-xs flex flex-col bg-gray-900 text-white" role="document" tabindex="-1">
        <div class="flex items-center justify-center h-20 border-b border-gray-700">
            <svg class="h-8 w-auto text-blue-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 16.5V9.75m0 0l3 3m-3-3l-3 3M6.75 19.5a4.5 4.5 0 01-1.41-8.775 5.25 5.25 0 0110.233-2.33 3 3 0 013.758 3.848A3.752 3.752 0 0118 19.5H6.75z" />
            </svg>
            <span class="ml-3 text-2xl font-semibold">Petro Kimia</span>
        </div>
        <nav class="flex-1 overflow-y-auto px-4 py-6">
            <?php
            if (function_exists('render_partial')) {
                render_partial('menu_petrokimia', ['mobile' => true]);
            } else {
                $mobile = true;
                include __DIR__ . '/menu_petrokimia.php';
            }
            ?>
        </nav>
        <div class="p-4 border-t border-gray-700">
            <?php if (is_superadmin()): ?>
                <a href="/dashboard/superadmin/dashboard.php" class="flex items-center px-4 py-3 text-gray-300 hover:bg-gray-700 hover:text-white rounded-lg">
                    Dashboard Superadmin
                </a>
            <?php endif; ?>
            <?php if (is_adminpk()): ?>
                <a href="../../logout.php" class="flex items-center px-4 py-3 text-gray-300 hover:bg-gray-700 hover:text-white rounded-lg">
                    Keluar
                </a>
            <?php endif; ?>
        </div>
    </div>
</div>