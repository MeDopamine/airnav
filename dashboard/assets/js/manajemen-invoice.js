$(document).ready(function () {
  ...
  var ajaxUrl = function () {
    ...
    return (
      "api/get_invoices.php" + (p ? "?periode=" + encodeURIComponent(p) : "")
    );
  };
        ...
        $.get(probeUrl)
          .done(function (resp) {
            if (resp && resp.ok) {
              ...
              Toast.fire({
                icon: "error",
                title: resp && resp.msg ? resp.msg : "Gagal menghasilkan file",
              });
            }
          })
          .fail(function () {
    ...
    $.get(probeUrl)
      .done(function (resp) {
        if (resp && resp.ok) {
          ...
          Toast.fire({
            icon: "error",
            title: resp && resp.msg ? resp.msg : "File invoice tidak tersedia",
          });
        }
      })
      .fail(function () {
  ...
  $(document).on("click", "#add-peserta-btn", function (e) {
    ...
    Swal.fire({
              ...
              if (selectedPeriode && selectedJenis) {
                ...
                $.get(
                  ...
                  function (resp) {
                    if (resp.ok) {
                      if (jmlPesertaEl)
                        jmlPesertaEl.value = resp.jumlah_peserta;
                      if (jmlPremilEl) {
      ...
      preConfirm: () => {
        ...
        const jenis_invoice =
          document.getElementById("swal-jenis_invoice").value;
        ...
        const linkPeserta =
          document.getElementById("swal-link_peserta").files[0];
    ...
    $.get("api/get_invoice_detail.php?id=" + invoiceId, function (resp) {
      ...
      Swal.fire({
        ...
        didOpen: async () => {
          ...
          document.getElementById("edit-jml_premi").value = data.total_premi
            .toString()
            .replace(/\B(?=(\d{3})+(?!\d))/g, ".");

          // Parse bulan dan tahun dari periode (YYYYMM)
          if (data.periode && data.periode.length === 6) {
            document.getElementById("edit-tahun").value = data.periode.slice(
              0,
              4
            );
          ...
          try {
            const perResponse = await fetch(
              "api/get_periode_jenis.php?mode=periods",
              {
                credentials: "include",
              }
            );
            ...
            if (perData.ok && perData.data) {
              const options =
                ...
                perData.data
                  .map(
                    (p) => `<option value="${p.periode}">${p.periode}</option>`
                  )
                  .join("");
          ...
          try {
            const jenisResponse = await fetch(
              "api/get_periode_jenis.php?mode=jenis",
              {
                credentials: "include",
              }
            );
            ...
            if (jenisData.ok && jenisData.data) {
              ...
              //   "<option></option>" +
              //   jenisData.data
              //     .map(`<option value="${(j) => j.jenis}">${j.jenis}</option>`)
              //     .join("");
              const options =
                "<option></option>" +
                jenisData.data
                  .map((j) => {
                    const value = j.jenis;
                    const name = jenisPremiMap[value] || value;
        ...
        preConfirm: () => {
          ...
          const jenis_invoice =
            document.getElementById("edit-jenis_invoice").value;
          ...
          const linkPeserta =
            document.getElementById("edit-link_peserta").files[0];
