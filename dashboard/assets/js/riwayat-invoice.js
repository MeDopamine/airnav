$(document).ready(function () {
    ...
    var ajaxUrl = function () {
        ...
        return 'api/get_invoice_approval_list.php' + (p ? ('?periode=' + encodeURIComponent(p)) : '');
    };
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    success: function (resp) {
                        ...
                        if (resp && resp.ok) {
                            Toast.fire({ icon: 'success', title: 'Invoice berhasil di-approve' });
                            table.ajax.reload();
                        } else {
                            Toast.fire({ icon: 'error', title: resp.msg || 'Gagal meng-approve invoice' });
                        }
                    },
                    error: function (xhr, status, error) {
                        console.error('Approve error:', error, xhr.responseText);
                        Toast.fire({ icon: 'error', title: 'Terjadi kesalahan saat meng-approve invoice' });
                    }
                });
            }
        });
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    success: function (resp) {
                        ...
                        if (resp && resp.ok) {
                            Toast.fire({ icon: 'success', title: 'Invoice berhasil di-reject' });
                            table.ajax.reload();
                        } else {
                            Toast.fire({ icon: 'error', title: resp.msg || 'Gagal reject invoice' });
                        }
                    },
                    error: function (xhr, status, error) {
                        console.error('Reject error:', error, xhr.responseText);
                        Toast.fire({ icon: 'error', title: 'Terjadi kesalahan saat reject invoice' });
                    }
                });
            }
        });
    ...
    $(document).on('click', '.btn-download', function (e) {
        ...
        $.get(probeUrl).done(function (resp) {
            if (resp && resp.ok) {
                ...
                Toast.fire({ icon: 'error', title: resp && resp.msg ? resp.msg : 'File tidak tersedia' });
            }
        }).fail(function () {
