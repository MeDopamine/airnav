...
(function (window) {
  ...
  function makeApproveBadge(status, id) {
    ...
    var badgeLabel = approved ? "Aktif" : "CLTP";
    return (
      '<span class="approve-btn cursor-pointer inline-flex items-center justify-center w-full h-full ' +
      badgeClass +
      '" style="min-width:110px;display:flex;align-items:center;justify-content:center;" data-id="' +
      (id || "") +
      '" data-status="' +
      approved +
      '">' +
  ...
  function rebuildPeriodeSelectFromSet() {
    ...
    arr.forEach(function (v) {
      $("#periode").append(
        $("<option>", { value: v, text: formatPeriodeReadable(v) })
      );
    });
  ...
  function resetApproveAllButton(btn) {
    try {
      btn
        .prop("disabled", false)
        .html('<i class="fa-solid fa-check-double mr-2"></i>Approve All');
    } catch (e) {
